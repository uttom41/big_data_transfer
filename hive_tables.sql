...
CREATE TABLE IF NOT EXISTS attendance_buffer (id INT, employee_id INT, card_id STRING, machine_id STRING, method INT, created TIMESTAMP, in_time STRING, out_time STRING, original_entry_time STRING, entry_time STRING, deleted TINYINT, posted TINYINT, wfh INT, dirty INT, updated TIMESTAMP, created_by INT, updated_by INT, status INT, reason STRING) PARTITIONED BY (in_date DATE, out_date DATE, entry_date DATE, original_entry_date DATE) STORED AS ORC
...
CREATE TABLE IF NOT EXISTS attendance_device_data (id BIGINT, employee_id INT, card_id STRING, machine_id STRING, method INT, created TIMESTAMP, entry_time STRING, original_entry_time STRING, deleted TINYINT, posted TINYINT, wfh INT, dirty INT, updated TIMESTAMP, created_by INT, updated_by INT, reason STRING) PARTITIONED BY (entry_date DATE, original_entry_date DATE) STORED AS ORC
...
CREATE TABLE IF NOT EXISTS inventories (id INT, name STRING, address_id INT, type INT, inventory_type INT, account_id INT, updated TIMESTAMP, updated_by INT, created TIMESTAMP, created_by INT, inactive TINYINT, deleted TINYINT, description STRING, branch_id INT, index INT, wastage_kpi INT, outline_id INT, project_id INT, parent_id INT, depth INT, order_number INT) PARTITIONED BY (`start` DATE, `end` DATE) STORED AS ORC
