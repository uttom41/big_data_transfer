    ...
    try:
        ...
        query = f"""
                SELECT
                    ...
                    CASE WHEN t2.qty_kgs IS NULL THEN 0 ELSE t2.qty_kgs END,
                    SUM(CASE WHEN inventory_journals.voucher_type IN ({voucher_types}) AND inventory_journals.delivered_quantity < 0 THEN inventory_journals.cost ELSE 0 END) AS delivery_out_cost,
                    SUM(CASE WHEN inventory_journals.voucher_type IN ({voucher_types}) AND inventory_journals.delivered_quantity < 0 THEN inventory_journals.delivered_quantity ELSE 0 END) AS delivery_out_quantity,
                    SUM(CASE WHEN inventory_journals.voucher_type IN ({voucher_types}) AND inventory_journals.delivered_quantity < 0 THEN inventory_journals.qty_kgs ELSE 0 END) AS out_qty_kgs,
                    SUM(CASE WHEN inventory_journals.voucher_type IN ({voucher_types}) AND inventory_journals.delivered_quantity > 0 THEN inventory_journals.cost ELSE 0 END) AS delivery_in_cost,
                    SUM(CASE WHEN inventory_journals.voucher_type IN ({voucher_types}) AND inventory_journals.delivered_quantity > 0 THEN inventory_journals.delivered_quantity ELSE 0 END) AS delivery_in_quantity,
                    SUM(CASE WHEN inventory_journals.voucher_type IN ({voucher_types}) AND inventory_journals.delivered_quantity > 0 THEN inventory_journals.qty_kgs ELSE 0 END) AS in_qty_kgs
                FROM
                    inventory_journals
                    LEFT JOIN (
                        SELECT
                        ...
                        WHERE
                            t3.entry_date < '{entry_date_start}' AND t3.inventory_id = {inventory_id} {additional_condition}
                        GROUP BY
                            t3.product_id,
                            t3.barcode,
                            t3.variant_1,
                ...
                WHERE
                    inventory_journals.entry_date > '{entry_date_start}' AND inventory_journals.entry_date <= '{entry_date_end}'
                    AND inventory_journals.inventory_id = {inventory_id} {additional_condition}
                GROUP BY
                    inventory_journals.barcode,
                    inventory_journals.variant_1,
                    inventory_journals.variant_2,
                    ...
                    inventory_journals.delivered_quantity
                """
